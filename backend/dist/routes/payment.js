...
router.post('/webhook', async (req, res) => {
    try {
        ...
        const expectedHash = crypto_1.default.createHash('sha1').update(str).digest('hex');
        if (expectedHash !== sha1_hash) {
            console.error('YuMoney webhook: invalid signature');
            return res.status(400).send('Bad signature');
        }
        // Parse label: inviteId__plan
        const [inviteId, plan] = label.split('__');
        if (!inviteId)
            return res.status(400).send('Bad label');
        await prisma_1.default.invitation.update({
            where: { id: inviteId },
            data: {
                status: 'paid',
...
router.get('/status/:inviteId', auth_1.authMiddleware, async (req, res) => {
    const invite = await prisma_1.default.invitation.findUnique({ where: { id: req.params.inviteId } });
    if (!invite || invite.userId !== req.userId)
